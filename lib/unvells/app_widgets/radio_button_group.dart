...
class _RadioButtonGroupState extends State<RadioButtonGroup> {
  ...
  Widget build(BuildContext context) {
    ...
    for(int i = 0; i < (widget.labels?.length ?? 0); i++){
      ...
      Radio rb = Radio(
        activeColor:Theme.of(context).colorScheme.onPrimary /*?? Theme.of(context).toggleableActiveColor*/,
        ...
        //one at a time
        onChanged: (widget.disabled != null && (widget.disabled?.contains(widget.labels?.elementAt(i)) ?? false)) ? null :
            (var index) => setState((){
      ...
      Text t = Text(
          ...
          // style: (widget.disabled != null && (widget.disabled?.contains(widget.labels?.elementAt(i)) ?? false)) ?
